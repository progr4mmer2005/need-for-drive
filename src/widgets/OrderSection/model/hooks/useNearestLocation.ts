...
export function useNearestLocation(cities: TCity[]): TNearestLocation | null {
  ...
  useEffect(() => {
    if (!cities.length || ranRef.current) return;
    ranRef.current = true;
    let cancelled = false;
    ...
    async function detect() {
      ...
      const sorted = geocodedCities
        ...
        .sort((a, b) => haversineKm(userLat, userLon, a.coords[0], a.coords[1])
      ...
      const nearestPickup = validPoints.length
        ? validPoints.reduce((best, curr) =>
            ...
            < haversineKm(userLat, userLon, best.coords[0], best.coords[1]) ? curr : best,
          ).point
